...
NeatDuck 是一个面向 Pokémon GO 活动规划的小型 Chrome 扩展。本文件随扩展包一起提供。它会在 LeekDuck Events 页面内嵌一个可配置面板，也可以从扩展图标打开独立页。v1.0.7 将原来的时间轴扩展为四个互相独立的 tab：Event Calendar、属性克制、宝可梦图鉴、极巨化。
