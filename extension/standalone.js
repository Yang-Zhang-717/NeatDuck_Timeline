...
(function(){
  ...
  function ensureTopbarTimeZoneSelect(){
    ...
    if(sel && !sel.options.length) sel.innerHTML=timeZoneOptionsHTML(state.displayTimeZone);
  ...
  function renderRulers(svg, rangeStart, rangeEnd){
    ...
    const mk=(group,y)=>{ const gg=document.createElementNS("http://www.w3.org/2000/svg","g"); gg.setAttribute("class","ld-grid"); group.appendChild(gg); pts.forEach(d=>{ const x=xScale(+d); if(!isFinite(x)) return; const l=document.createElementNS("http://www.w3.org/2000/svg","line"); l.setAttribute("x1",x); l.setAttribute("x2",x); l.setAttribute("y1",y); l.setAttribute("y2",(C.HEIGHT-C.TOPBAR_H)-C.BOTTOM_RULER_H); gg.appendChild(l); const t=document.createElementNS("http://www.w3.org/2000/svg","text"); t.setAttribute("x",x+3); t.setAttribute("y",y-6); t.textContent=fmtTick(rule,d); group.appendChild(t); }); };
    ...
    pts.forEach(d=>{ const x=xScale(+d); if(!isFinite(x)) return; const l=document.createElementNS("http://www.w3.org/2000/svg","line"); l.setAttribute("x1",x); l.setAttribute("x2",x); l.setAttribute("y1",C.TIMELINE_TOP_PAD || 0); l.setAttribute("y2",(C.HEIGHT-C.TOPBAR_H)-C.BOTTOM_RULER_H); gg.appendChild(l); const t=document.createElementNS("http://www.w3.org/2000/svg","text"); t.setAttribute("x",x+3); t.setAttribute("y",(C.HEIGHT-C.TOPBAR_H)-6); t.textContent=fmtTick(rule,d); bottom.appendChild(t); });
  }
  ...
  function fitRangeToEvents(){ setRangePreset("week"); }
  ...
  async function init(){
    ...
    if(todayBtn && !todayBtn.__ldtBound) todayBtn.addEventListener("click", ()=>{ setRangePreset("today"); updateViewMode(); });
    if(todayBtn) todayBtn.__ldtBound = true;
    const weekBtn = document.querySelector('[data-act="reset-week"]');
    if(weekBtn && !weekBtn.__ldtBound) weekBtn.addEventListener("click", ()=>{ setRangePreset("week"); updateViewMode(); });
    if(weekBtn) weekBtn.__ldtBound = true;
    const monthBtn = document.querySelector('[data-act="reset-month"]');
    if(monthBtn && !monthBtn.__ldtBound) monthBtn.addEventListener("click", ()=>{ setRangePreset("month"); updateViewMode(); });
