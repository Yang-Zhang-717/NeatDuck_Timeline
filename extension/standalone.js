...
(function(){
  ...
  let state={ lang:LDT_Core.DEFAULT_LANG, events:[], colors:{...LDT_Core.DEFAULT_COLORS}, settings:LDT_Core.normalizeSettings({}), rangeStart:null, rangeEnd:null, viewMode:"linear", displayTimeZone:LDT_Core.DEFAULT_DISPLAY_TIME_ZONE, pokemonDB:null, detailsCache:{}, selectedIds:new Set(), remoteUrl:LDT_Core.DEFAULT_REMOTE_URL, activeTab:"calendar" };
  ...
  function setActiveTab(tab){ state.activeTab=tab||'calendar'; updateViewMode(); }
  function updateViewMode(){
    updateTabButtons();
    const svg=document.getElementById("ld-timeline-svg"); const mg=document.getElementById("ld-monthgrid"); const left=document.getElementById('ld-left'); const wrap=document.getElementById('ld-timeline-wrapper');
  ...
  async function init(){
    ...
    document.querySelectorAll("#ld-tabs [data-tab]").forEach(btn=>{ if(!btn.__ldtBound){ btn.addEventListener("click", ev=>{ ev.preventDefault(); setActiveTab(btn.getAttribute("data-tab")); }); btn.__ldtBound=true; } });
